    ...
    private void BtnStart_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                // 解析输入
                int[] pages = TxtPages.Text.Split(new[] { ',', ' ', ';' }, StringSplitOptions.RemoveEmptyEntries)
                ...
                UpdateVisualization();
                TxtStats.Text = $"总页数: {pages.Length} | 缺页次数: {_records.Count(r => !r.IsHit)} | 缺页率: {(_records.Count(r => !r.IsHit) * 100.0 / pages.Length):F1}%";
        ...
        private void UpdateVisualization()
        {
            ...
            var vms = _records.Select((r, idx) => new StepViewModel
            {
                ...
                BgColor = r.IsHit ? new SolidColorBrush(Color.FromRgb(232, 245, 233)) :   // 绿色背景-命中
                                  new SolidColorBrush(Color.FromRgb(255, 235, 238)),      // 红色背景-缺页
                FgColor = r.IsHit ? new SolidColorBrush(Color.FromRgb(46, 125, 50)) :
        ...
        {
            if (_records == null || step < 0 || step >= _records.Count) return;
            _currentStep = step;
        ...
        {
            if (_isAutoPlaying) { StopAutoPlay(); return; }
            if (_records == null) { BtnStart_Click(sender, e); return; }
        ...
        {
            if (_currentStep >= _records.Count - 1) { StopAutoPlay(); return; }
